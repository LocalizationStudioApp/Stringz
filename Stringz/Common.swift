...
class Common {
    ...
    static func inputAlert(
        ...
        window: NSWindow? = nil,
        completion: ((NSApplication.ModalResponse, String) -> Void)? = nil
    ) -> NSAlert {
        let alert = NSAlert()
        alert.messageText = message
        alert.informativeText = informative
    ...
    static func showFeatureNotSupported(window: NSWindow? = nil) {
        _ = Common.alert(message: "This featrue will be supported very soon.", positiveButton: "OK", window: window)
    }
