...
class MainWindowController: NSWindowController, NSWindowDelegate, EditorManagerDelegate {
    ...
    @IBOutlet var addLanguagePopUpButton: NSPopUpButton!
    @IBOutlet var searchField: InfoSearchField!
    ...
    required init?(coder: NSCoder) {
        ...
        self.editorManager = EditorManager(delegate: self)
    }
    ...
    @objc func performAddLanguage(_ sender: Any) {
        ...
        addLanguage(language, at: 0, with: nil, in: selectedLocalizableIndex)
    }

    @objc func performAddString(_ sender: Any) {
        guard let selectedLocalizable = selectedLocalizable else { return }

        let viewController = AddStringViewController.instantiateFromStoryboard()
        viewController.validationHandler = { key in
            !selectedLocalizable.valueSets.contains(key: key)
        }
    ...
    func closeDocument() -> Bool {
        if !documentUpdated {
            ...
            var projectName = ""
            if let name = currentProjectPath?.lastComponent {
                projectName = " \"\(name)\""
            }
...
extension MainWindowController {
    func loadProject() {
        guard let currentProjectPath = currentProjectPath else {
        ...
        if UserDefaults.generalAutoload {
            loadLocalizables(localizables.sorted(by: { $0 < $1 }))
        }
    ...
    func unloadLocalizables(_ localizables: [Localizable]) {
        for localizable in localizables {
            ...
            if self.localizables.firstIndex(of: localizable) == selectedLocalizableIndex {
                selectLocalizable(selectedLocalizableIndex, reload: true)
            }
        }
    }

    func selectLocalizable(_ localizableIndex: Int, reload: Bool = false) {
        guard reload || selectedLocalizableIndex != localizableIndex else { return }
        selectedLocalizableIndex = localizableIndex
    ...
    func addValueSets(_ valueSets: [ValueSet], to localizableIndex: Int? = nil, registerActionName: Bool = true) {
        let localizableIndex = localizableIndex ?? selectedLocalizableIndex
        guard localizableIndex >= 0, localizables.count > localizableIndex else { return }

        // Add value sets to the localizable
        let localizable = localizables[localizableIndex]
        localizable.valueSets.append(contentsOf: valueSets)
    ...
    func updateValueSet(_ valueSet: ValueSet, with identifier: String, newValue: String, in localizableIndex: Int? = nil, registerActionName: Bool = true) {
        ...
        if identifier == "key" {
            ...
            if localizables.first(where: { $0.valueSets.contains(valueSet) })?.localizableType == .config {
                importerOptions.plistKeys.appendIfDoesntExist(newValue)
            }
        } else if identifier == "comment" {
    ...
    func addLanguage(_ language: Language, at fileIndex: Int, with data: Data?, in localizableIndex: Int? = nil) {
        ...
        selectLocalizable(localizableIndex)
    ...
    func localizeFile(_ localizableIndex: Int? = nil, registerActionName: Bool = true) {
        guard let currentProjectPath = currentProjectPath else { return }
        let sidebarViewController = (contentViewController as! MainViewController).sidebarViewController

        let localizableIndex = localizableIndex ?? selectedLocalizableIndex
        var localizable = localizables[localizableIndex]
        ...
        let newIndexInParent = localizables
        ...
        loadLocalizables([localizable])
        selectLocalizable(localizableIndex, reload: true)
    ...
    func unlocalizeFile(_ localizableIndex: Int? = nil, registerActionName: Bool = true) {
        ...
        unloadLocalizables([localizable])
    ...
    func saveAll() {
        dirtyFilesUUids.uniqued().forEach { uuid in
            guard
...
extension MainWindowController {
    private func setupListeners() {
        ...
        watchFileSubscriber = NotificationCenter.default.publisher(for: .WatchFile)
            .receive(on: RunLoop.main)
            .sink(receiveValue: watchFileListener)

        unwatchFileSubscriber = NotificationCenter.default.publisher(for: .UnwatchFile)
            .receive(on: RunLoop.main)
            .sink(receiveValue: unwatchFileListener)
    }

    private func cancelListeners() {
        watchFileSubscriber?.cancel()
        watchFileSubscriber = nil

        unwatchFileSubscriber?.cancel()
        unwatchFileSubscriber = nil

        saveFileSubscribers.forEach { $0.value.cancel() }
        saveFileSubscribers.removeAll()

        print("cancel - watcher count: \(saveFileSubscribers.count)")
    }
}
